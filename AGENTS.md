...
- 涉及短剧角色照、场景图、关键帧、分镜、视频提示词或 image2 生图前，必须先读取 `short-drama/AGENTS.md`。
...
- `short-drama/assets/characters/_asset-index.md`：已生成角色照资产索引。
...
任何角色照、场景图、关键帧、分镜和视频提示词生成前，必须先读取：
...
不得跳过角色 ID、年龄版本和资产索引检查直接生图。已有 `approved` 角色照时，不得重复生成同一角色同一年龄版本，除非用户明确要求重做或迭代。
