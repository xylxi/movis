...
├── source/                       # 来源说明：原创/改编/参考、授权状态和改编边界
├── novel/                        # 小说母本：总纲、章节规划和已完成正文
...
2. 来源说明：在 `source/source-note.md` 记录原创/改编/参考、来源材料、授权状态和改编边界。
3. 小说母本：在 `novel/outline.md` 建立小说总纲和章节规划；小说正文可以先写当前集需要的部分，但不能缺正文入口。
4. 项目 bible：整理世界观、人物关系、核心冲突、爽点机制和连续性规则。
5. 单集剧本：从小说母本拆成可拍场次、动作、对白、道具变化和情绪转折。
6. 剧本审查：检查因果、爽点、可拍性、节奏、人物动机和结尾钩子。
7. 素材 brief：规划人物设定图、场景图、道具图和必要关键帧。
8. 生图与索引：素材进入项目 `assets/` 根目录，用户确认后标记可用。
9. 镜头节拍：把剧本拆成 Seedance 可执行的时间线、动作、镜头和声音。
10. 视频交付：输出可直接输入视频模型的提示词、素材引用和禁止项。
...
- 没有 `source/source-note.md`、`novel/outline.md` 和当前集依赖的小说正文，不进入正式剧本；小说可以先写一部分，但来源和小说流程不能省略。
