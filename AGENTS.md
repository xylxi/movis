...
- 涉及短剧人物设定图、角色照、场景图、关键帧、分镜、视频提示词或 image2 生图前，必须先读取 `short-drama/AGENTS.md`。
...
任何人物设定图、角色照、场景图、关键帧、分镜和视频提示词生成前，必须先读取：

1. `short-drama/AGENTS.md`
2. `short-drama/adaptation-control.md`
3. `short-drama/characters/character-bible.md`
4. 对应角色规格文件，例如 `short-drama/characters/xu-xing.md`
5. 对应集数出场表，例如 `short-drama/characters/episode-rosters/01-glass-crack.md`
6. `short-drama/assets/characters/_asset-index.md`

不得跳过角色 ID、年龄版本和资产索引检查直接生图。角色视觉资产默认优先生成人物设定图，用三视图、表情、服装/道具拆解、配色和姿态锁定人物形态；单张角色照只作为补充。已有 `approved` 人物设定图或角色照时，不得重复生成同一角色同一年龄版本，除非用户明确要求重做或迭代。
